    ...
    return (
        <header className='w-full sticky top-0 z-50'>
            <div className='bg-[#34967C] py-2 w-full text-white flex justify-center items-center gap-2'>
                <Image width={24} height={24} src={CASE_MINIMALISTIC} alt="case image" />
                <p className='text-sm'>Overseas trip? Get the latest information on travel guides</p>
                <Button label='More Info' className='bg-black rounded-2xl text-white text-sm px-3 py-1' />
            </div>
            <div className='bg-white w-full h-[40px] flex  justify-around items-center py-2'>
                ...
                <div className='flex items-center gap-1 mt-2'>
                    <Button label="Sign In" className='bg-[#34967C] text-white text-sm px-5 py-2  rounded-4xl' />
                    <Button label="Sign Up" className=' border bg-white py-2 px-5 text-black text-sm  rounded-4xl border-gray-300' />

                </div>
            </div>
            <div className=' bg-white flex justify-center gap-5 py-4 overflow-x-auto px-4'>
                {accommodationTypes.map((item , index)=>(
                    <div key={index} className='flex flex-col items-center text-sm text-gray-700 hover:text-[#34967C] cursor-pointer'>
                        <Image  src={item.image} alt={item.title} width={34} height={34}  loading="lazy"/>
                        <span className='mt-1 text-[12px]'>{item.title}</span>
                    </div>
                ))}
